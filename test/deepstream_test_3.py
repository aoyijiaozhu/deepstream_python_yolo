...
def pgie_src_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            ...
            # 获取类别名称
            if class_id < len(pgie_classes_str):
                class_name = pgie_classes_str[class_id]
            else:
                class_name = f"Unknown_{class_id}"

            # 计数
            if class_name in obj_counter:
        ...
        if not silent:
            # 打印 80 类统计信息
            print(f"Frame Number={frame_number} Number of Objects={num_rects} Details={obj_counter}")
    ...
    if no_display:
        ...
        if platform_info.is_integrated_gpu():
            ...
            if platform_info.is_platform_aarch64():
                ...
                print("Creating EGLSink (Windowed Display) \n")
                # 这里的 EGLSink 就是导致你黑屏的源头
                # 必须配合 export QT_X11_NO_MITSHM=1 使用
...
def parse_args():
    ...
    parser.add_argument("-i", "--input", help="Path to input streams", nargs="+", metavar="URIs", default=["a"],
                        required=True)
    parser.add_argument("-c", "--configfile", metavar="config_location.txt", default=None,
